...
def main():
 f=L(ROOT/'data/imports/lebanon/fixtures/current_hierarchy.json');e=[r for r in read_jsonl(ROOT/'data/entities/entities.jsonl') if r.get('country_code')=='LB'];ids={r['id'] for r in e};families={'entities':e,'relationships':[r for r in read_jsonl(ROOT/'data/relationships/relationships.jsonl') if r.get('child_id') in ids],'claims':[r for r in read_jsonl(ROOT/'data/claims/claims.jsonl') if r.get('subject_id') in ids],'denominators':[r for r in read_jsonl(ROOT/'data/coverage/denominators.jsonl') if r.get('country_code')=='LB'],'coverage':[r for r in read_jsonl(ROOT/'data/coverage/coverage.jsonl') if r.get('country_code')=='LB']};sids={'SRC-LB-CAS-26-DISTRICTS-2019','SRC-LB-MOIM-ADMINISTRATION-2026','SRC-LB-LAW-52-2017','SRC-LB-CULTURE-MIRROR-2026'};families['sources']=[L(p) for p in (ROOT/'data/sources').glob('*.json') if L(p).get('id') in sids];E={r['id']:r for r in e};R=families['relationships'];find=[]
 ...
 if len(depth)!=20 or any(c.get('published') for c in depth):find.append({'severity':'P1','record_id':'depth','message':'20 unpublished cultural depth claims required'})
 if any(c.get('predicate')=='food_dish' and c['value']['data'].get('name')=='التبولة' and c.get('classification')!='shared' for c in depth):find.append({'severity':'P1','record_id':'التبولة','message':'shared dish must not become exclusive'})
