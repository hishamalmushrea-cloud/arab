...
def main():
 f=L(ROOT/'data/imports/oman/fixtures/legal_hierarchy_2022.json');e=[r for r in read_jsonl(ROOT/'data/entities/entities.jsonl') if r.get('country_code')=='OM'];ids={r['id'] for r in e};families={'entities':e,'aliases':[r for r in read_jsonl(ROOT/'data/aliases/aliases.jsonl') if r.get('entity_id') in ids],'relationships':[r for r in read_jsonl(ROOT/'data/relationships/relationships.jsonl') if r.get('child_id') in ids],'claims':[r for r in read_jsonl(ROOT/'data/claims/claims.jsonl') if r.get('subject_id') in ids],'denominators':[r for r in read_jsonl(ROOT/'data/coverage/denominators.jsonl') if r.get('country_code')=='OM'],'coverage':[r for r in read_jsonl(ROOT/'data/coverage/coverage.jsonl') if r.get('country_code')=='OM']};sids={'SRC-OM-RD-36-2022-GOVERNORATES','SRC-OM-ONA-63-WILAYATS-2022','SRC-OM-NCSI-ADMIN-CODES-2026','SRC-OM-CULTURE-MIRROR-2026'};families['sources']=[L(p) for p in (ROOT/'data/sources').glob('*.json') if L(p).get('id') in sids];find=[];E={r['id']:r for r in e};R=families['relationships']
 ...
 if len(depth)!=21 or any(c.get('published') for c in depth):find.append({'severity':'P1','record_id':'depth','message':'21 unpublished cultural depth claims required'})
 for nm in ['القبولي','الهريس','الثريد','المضروبة']:
  if any(c.get('predicate')=='food_dish' and c['value']['data'].get('name')==nm and c.get('classification')!='shared' for c in depth):find.append({'severity':'P1','record_id':nm,'message':'shared dish must not become exclusive'})
