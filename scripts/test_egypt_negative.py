...
def main():
 ...
 r('EG_MARKAZ_UNDER_CAIRO',lambda d:d['entities'].append({**d['entities'][-1],'id':'ENT-EG-MARKAZ-X','entity_type':'eg_markaz'}),'EG_PREMATURE_LOWER')
 ...
 ok=all(x['detected'] for x in o);write_json(ROOT/'reports/egypt_negative_tests.json',{'schema_version':'2.0.0','country_code':'EG','status':'PASS' if ok else 'FAIL','required':8,'detected':sum(x['detected'] for x in o),'mutations':o});print(o);return 0 if ok else 1
