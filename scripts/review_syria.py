...
def main():
 f=L(ROOT/'data/imports/syria/fixtures/governorates_2026.json');e=[r for r in read_jsonl(ROOT/'data/entities/entities.jsonl') if r.get('country_code')=='SY'];ids={r['id'] for r in e};families={'entities':e,'aliases':[r for r in read_jsonl(ROOT/'data/aliases/aliases.jsonl') if r.get('entity_id') in ids],'relationships':[r for r in read_jsonl(ROOT/'data/relationships/relationships.jsonl') if r.get('child_id') in ids],'claims':[r for r in read_jsonl(ROOT/'data/claims/claims.jsonl') if r.get('subject_id') in ids],'denominators':[r for r in read_jsonl(ROOT/'data/coverage/denominators.jsonl') if r.get('country_code')=='SY'],'coverage':[r for r in read_jsonl(ROOT/'data/coverage/coverage.jsonl') if r.get('country_code')=='SY']};sids={'SRC-SY-SIA-ADMIN-DIVISIONS-2026','SRC-SY-SANA-GOVERNORATE-NAV-2026','SRC-SY-CBS-GOVERNORATES-2010','SRC-SY-CULTURE-MIRROR-2026'};families['sources']=[L(p) for p in (ROOT/'data/sources').glob('*.json') if L(p).get('id') in sids];E={r['id']:r for r in e};C=families['claims'];find=[]
 ...
 for i,n in [('COV-SY-DISTRICTS',68),('COV-SY-SUBDISTRICTS',227)]:
  ...
  depth=[c for c in families['claims'] if c.get('predicate') in {'language_presence','dialect_profile','food_dish','clothing_item'}]
 if len(depth)!=24 or any(c.get('published') for c in depth):find.append({'severity':'P1','record_id':'depth','message':'24 unpublished cultural depth claims required'})
 if any(c.get('predicate')=='food_dish' and c['value']['data'].get('name')=='الكبة' and c.get('classification')!='shared' for c in depth):find.append({'severity':'P1','record_id':'الكبة','message':'shared dish must not become exclusive'})
