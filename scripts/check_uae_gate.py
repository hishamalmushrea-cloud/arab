...
def main():
 ...
 d=v.get('checks',{}).get('depth',{})
 g.require(d.get('claims')==70 and d.get('published')==33 and d.get('unpublished')==37 and d.get('national_elements')==1 and d.get('deferred_files')==13 and d.get('tentative_files')==15 and d.get('places')==3 and {l.get('layer') for l in m.get('pilot_layers',[])}>=DEPTH_LAYERS and m.get('snapshot',{}).get('snapshot_id')=='SNP-AE-PILOT-20260815' and set(snaps)=={'SNP-AE-PILOT-20260815','SNP-AE-DEPTH-20260923'} and snaps['SNP-AE-DEPTH-20260923'].get('captured_at')=='2026-09-23','uae_depth_cycle_shape',f"claims={d.get('claims')}/{d.get('published')} published, national={d.get('national_elements')}, deferred={d.get('deferred_files')}, tentative={d.get('tentative_files')}, places={d.get('places')}, layers={len(m.get('pilot_layers',[]))}, snapshots={sorted(snaps)}")
 ...
 g.require(n.get('status')=='PASS' and n.get('detected')==n.get('required')==28,'uae_required_mutations',f"detected={n.get('detected')}/{n.get('required')}")
