...
MANAGED=['data/entities/entities.jsonl','data/aliases/aliases.jsonl','data/relationships/relationships.jsonl','data/claims/claims.jsonl','data/coverage/denominators.jsonl','data/coverage/coverage.jsonl','data/snapshots/snapshots.jsonl','manifests/AE.yml','data/cultural/uae_domain_status.json','data/imports/uae/source_catalog.json','data/imports/uae/snapshot_manifest.json','data/imports/uae/fixtures/administrative_profile.json','data/imports/uae/fixtures/cultural_claims.json','data/imports/uae/fixtures/cultural_depth_2026.json']
...
def main():
 ...
 d=v.get('checks',{}).get('depth',{}).get('detail',{});g.require(d.get('claims')==70 and d.get('published')==33 and d.get('unpublished')==37 and d.get('national_elements')==1 and d.get('deferred_files')==13 and d.get('tentative_files')==15 and d.get('places')==3,'uae_depth_cycle_shape',f"depth claims={d.get('claims')} published={d.get('published')} national={d.get('national_elements')} deferred={d.get('deferred_files')} tentative={d.get('tentative_files')} places={d.get('places')}")
 ...
 g.require({l.get('layer') for l in m.get('pilot_layers',[])}>=DEPTH_LAYERS and m.get('depth_status')=='depth_cycle_1' and m.get('depth_snapshot',{}).get('snapshot_id')=='SNP-AE-DEPTH-20260923' and m.get('snapshot',{}).get('snapshot_id')=='SNP-AE-PILOT-20260815','uae_depth_layers_declared',f"layers={len(m.get('pilot_layers',[]))}, pilot={m.get('snapshot',{}).get('snapshot_id')}, depth={m.get('depth_snapshot',{}).get('snapshot_id')}")
