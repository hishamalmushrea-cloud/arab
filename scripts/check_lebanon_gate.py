#!/usr/bin/env python3
"""Lebanon production release gate, layered after the accepted Phase 5 gate."""
...
def file_hashes(): return {p: sha(ROOT / p) for p in LBNAGED}

def source_hashes():
    result = {}
    for path in sorted((ROOT / "data/sources").glob("*.json")):
        row = json.loads(path.read_text(encoding="utf-8"))
        if row.get("country_codes") == ["LB"]: result[str(path.relative_to(ROOT))] = sha(path)
    return result

def non_lebanon_hash() -> str:
    entities = read_jsonl(ROOT / "data/entities/entities.jsonl")
    kw_ids = {r["id"] for r in entities if r.get("country_code") == "LB"}
    payload = {
        ...
        "denominators": [r for r in read_jsonl(ROOT / "data/coverage/denominators.jsonl") if r["country_code"] != "LB"],
        "coverage": [r for r in read_jsonl(ROOT / "data/coverage/coverage.jsonl") if r["country_code"] != "LB"],
        "snapshots": [r for r in read_jsonl(ROOT / "data/snapshots/snapshots.jsonl") if not r["id"].startswith("SNP-LB-")],
    }
...
def main() -> int:
    ...
    gate.require(before_files == after_files, "lebanon_import_idempotence", f"all {len(LBNAGED)} managed hashes unchanged={before_files == after_files}")
    gate.require(before_other == after_other, "non_lebanon_preservation", f"canonical non-Lebanon SHA-256 unchanged={after_other}")
    gate.command("general_validation", [sys.executable, "scripts/validate.py"])
    gate.command("lebanon_semantic_validation", [sys.executable, "scripts/validate_lebanon.py"])
    gate.command("lebanon_negative_tests", [sys.executable, "scripts/test_lebanon_negative.py"])
    sample_path = ROOT / "data/review/lebanon_review_samples.json"
    before_sample = sha(sample_path)
    gate.command("lebanon_review_sample_refresh", [sys.executable, "scripts/build_lebanon_review_samples.py"])
    gate.require(before_sample == sha(sample_path), "lebanon_review_sample_idempotence", f"full-population sample unchanged={before_sample == sha(sample_path)}")
    before_review = sha(ROOT / "reports/lebanon_independent_review.json")
    gate.command("lebanon_independent_review", [sys.executable, "scripts/review_lebanon.py"])
    gate.require(before_review == sha(ROOT / "reports/lebanon_independent_review.json"), "lebanon_review_idempotence", f"independent review unchanged={before_review == sha(ROOT / 'reports/lebanon_independent_review.json')}")
    gate.command("generated_freshness", [sys.executable, "scripts/generate.py", "--check"])
    validation = json.loads((ROOT / "reports/lebanon_validation.json").read_text(encoding="utf-8"))
    ...
    gate.require(not status, "lebanon_clean_worktree", f"git worktree clean={not status}")
    report = {"schema_version": "2.0.0", "country_code": "LB", "snapshot_date": "2026-08-16", "status": "pass" if not gate.errors else "fail", "checks": gate.checks, "errors": gate.errors}
    write_json(ROOT / "reports/lebanon_gate.json", report)
    ...
    if gate.errors:
        ...
        return 1
    print(f"Lebanon production gate passed ({len(gate.checks)} checks).")
    return 0
