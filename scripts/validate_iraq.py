...
def data():
 e=[r for r in read_jsonl(ROOT/'data/entities/entities.jsonl') if r.get('country_code')=='IQ'];ids={r['id'] for r in e};sids={'SRC-IQ-COSIT-GOVERNORATES-2023','SRC-IQ-LAW-7-HALABJA-2025','SRC-IQ-DECREE-21-HALABJA-2025','SRC-IQ-KRSO-FOUR-GOVERNORATES','SRC-IQ-QADA-LISTS-MIRROR-2026'}
...
def validate(d):
 ...
 qd=[r for r in E.values() if r['entity_type']=='iq_district']
 if sum(r['entity_type']=='iq_governorate' for r in E.values())!=19 or len(qd)!=31 or len(E)!=51:x('IQ_COUNTS','entities','country+19+31 qadas')
 ...
 if want!=got:x('IQ_QADA_FIXTURE','qadas','qada (parent,name,status) set must equal the checksum-bound 31-qada fixture')
 qpop=[c for c in C if c.get('predicate')=='population' and c.get('subject_id') in {r['id'] for r in qd}]
 if len(qpop)!=3 or any(c.get('published') for c in qpop):x('IQ_QADA_POP','claims','3 unpublished probable Basra census qada populations')
 ...
 if any(r['entity_type']=='iq_subdistrict' for r in E.values()):x('IQ_PREMATURE_LOWER','entities','subdistrict deferred')
