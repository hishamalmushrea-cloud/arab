...
MANAGED=['data/entities/entities.jsonl','data/aliases/aliases.jsonl','data/relationships/relationships.jsonl','data/claims/claims.jsonl','data/coverage/denominators.jsonl','data/coverage/coverage.jsonl','data/snapshots/snapshots.jsonl','manifests/SO.yml','data/cultural/somalia_domain_status.json','data/imports/somalia/source_catalog.json','data/imports/somalia/snapshot_manifest.json','data/imports/somalia/fixtures/cultural_depth_2026.json']
class Gate:
...
def main():
 g=Gate();before=source_hashes();g.command('somalia_source_refresh',[sys.executable,'scripts/build_somalia_sources.py']);after=source_hashes();g.require(len(after)==18 and before==after,'somalia_source_idempotence',f'atomic sources={len(after)}/18, unchanged={before==after}')
