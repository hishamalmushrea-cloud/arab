#!/usr/bin/env python3
"""Morocco production release gate, layered after the accepted Phase 5 gate."""
...
def source_hashes():
    ...
    for path in sorted((ROOT / "data/sources").glob("*.json")):
        ...
        if row.get("country_codes") == ["MA"]: result[str(path.relative_to(ROOT))] = sha(path)
    return result

def non_morocco_hash() -> str:
    entities = read_jsonl(ROOT / "data/entities/entities.jsonl")
    kw_ids = {r["id"] for r in entities if r.get("country_code") == "MA"}
    payload = {
        ...
        "denominators": [r for r in read_jsonl(ROOT / "data/coverage/denominators.jsonl") if r["country_code"] != "MA"],
        "coverage": [r for r in read_jsonl(ROOT / "data/coverage/coverage.jsonl") if r["country_code"] != "MA"],
        "snapshots": [r for r in read_jsonl(ROOT / "data/snapshots/snapshots.jsonl") if not r["id"].startswith("SNP-MA-")],
    }
...
def main() -> int:
    ...
    gate.require(before_files == after_files, "morocco_import_idempotence", f"all {len(MANAGED)} managed hashes unchanged={before_files == after_files}")
    gate.require(before_other == after_other, "non_morocco_preservation", f"canonical non-Morocco SHA-256 unchanged={after_other}")
    gate.command("general_validation", [sys.executable, "scripts/validate.py"])
    gate.command("morocco_semantic_validation", [sys.executable, "scripts/validate_morocco.py"])
    gate.command("morocco_negative_tests", [sys.executable, "scripts/test_morocco_negative.py"])
    sample_path = ROOT / "data/review/morocco_review_samples.json"
    before_sample = sha(sample_path)
    gate.command("morocco_review_sample_refresh", [sys.executable, "scripts/build_morocco_review_samples.py"])
    gate.require(before_sample == sha(sample_path), "morocco_review_sample_idempotence", f"full-population sample unchanged={before_sample == sha(sample_path)}")
    before_review = sha(ROOT / "reports/morocco_independent_review.json")
    gate.command("morocco_independent_review", [sys.executable, "scripts/review_morocco.py"])
    gate.require(before_review == sha(ROOT / "reports/morocco_independent_review.json"), "morocco_review_idempotence", f"independent review unchanged={before_review == sha(ROOT / 'reports/morocco_independent_review.json')}")
    gate.command("generated_freshness", [sys.executable, "scripts/generate.py", "--check"])
    validation = json.loads((ROOT / "reports/morocco_validation.json").read_text(encoding="utf-8"))
    ...
    gate.require(not status, "morocco_clean_worktree", f"git worktree clean={not status}")
    report = {"schema_version": "2.0.0", "country_code": "MA", "snapshot_date": "2026-08-16", "status": "pass" if not gate.errors else "fail", "checks": gate.checks, "errors": gate.errors}
    write_json(ROOT / "reports/morocco_gate.json", report)
    ...
    if gate.errors:
        ...
        return 1
    print(f"Morocco production gate passed ({len(gate.checks)} checks).")
    return 0
