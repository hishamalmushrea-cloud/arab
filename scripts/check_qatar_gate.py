#!/usr/bin/env python3
"""Qatar production release gate, layered after the accepted Phase 5 gate."""
...
MANAGED = [
    ...
    "data/snapshots/snapshots.jsonl", "manifests/QA.yml", "data/cultural/qatar_domain_status.json",
    "data/imports/qatar/source_catalog.json",
]
...
def source_hashes():
    ...
    for path in sorted((ROOT / "data/sources").glob("*.json")):
        ...
        if row.get("country_codes") == ["QA"]: result[str(path.relative_to(ROOT))] = sha(path)
    return result

def non_qatar_hash() -> str:
    entities = read_jsonl(ROOT / "data/entities/entities.jsonl")
    kw_ids = {r["id"] for r in entities if r.get("country_code") == "QA"}
    payload = {
        ...
        "denominators": [r for r in read_jsonl(ROOT / "data/coverage/denominators.jsonl") if r["country_code"] != "QA"],
        "coverage": [r for r in read_jsonl(ROOT / "data/coverage/coverage.jsonl") if r["country_code"] != "QA"],
        "snapshots": [r for r in read_jsonl(ROOT / "data/snapshots/snapshots.jsonl") if not r["id"].startswith("SNP-QA-")],
    }
...
def main() -> int:
    ...
    gate.require(before_files == after_files, "qatar_import_idempotence", f"all {len(MANAGED)} managed hashes unchanged={before_files == after_files}")
    gate.require(before_other == after_other, "non_qatar_preservation", f"canonical non-Qatar SHA-256 unchanged={after_other}")
    gate.command("general_validation", [sys.executable, "scripts/validate.py"])
    gate.command("qatar_semantic_validation", [sys.executable, "scripts/validate_qatar.py"])
    gate.command("qatar_negative_tests", [sys.executable, "scripts/test_qatar_negative.py"])
    sample_path = ROOT / "data/review/qatar_review_samples.json"
    before_sample = sha(sample_path)
    gate.command("qatar_review_sample_refresh", [sys.executable, "scripts/build_qatar_review_samples.py"])
    gate.require(before_sample == sha(sample_path), "qatar_review_sample_idempotence", f"full-population sample unchanged={before_sample == sha(sample_path)}")
    before_review = sha(ROOT / "reports/qatar_independent_review.json")
    gate.command("qatar_independent_review", [sys.executable, "scripts/review_qatar.py"])
    gate.require(before_review == sha(ROOT / "reports/qatar_independent_review.json"), "qatar_review_idempotence", f"independent review unchanged={before_review == sha(ROOT / 'reports/qatar_independent_review.json')}")
    gate.command("generated_freshness", [sys.executable, "scripts/generate.py", "--check"])
    validation = json.loads((ROOT / "reports/qatar_validation.json").read_text(encoding="utf-8"))
    ...
    if gate.errors:
        ...
        return 1
    print(f"Qatar production gate passed ({len(gate.checks)} checks).")
    return 0
