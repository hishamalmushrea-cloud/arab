...
def data():
 e=[r for r in read_jsonl(ROOT/'data/entities/entities.jsonl') if r.get('country_code')=='YE']; ids={r['id'] for r in e}; sids={'SRC-YE-CSO-ADMIN-DEFINITION-LEGACY','SRC-YE-NIC-GOVERNORATES-LEGACY','SRC-YE-LAW-31-SOCOTRA-REPORT-2013','SRC-YE-NIC-DISTRICT-CATALOGUE-MIRROR','SRC-YE-CENSUS-2004-LEGACY-FRAME'}
...
def validate(d):
 ...
 gov=[r for r in E.values() if r['entity_type']=='ye_governorate']; cap=[r for r in E.values() if r['entity_type']=='ye_capital_municipality']; dis=[r for r in E.values() if r['entity_type']=='ye_district']
 if len(E)!=356 or len(gov)!=21 or len(cap)!=1 or len(dis)!=333: x('YE_COUNTS','entities','country + 21 governorates + 1 capital municipality + 333 districts')
 ...
 expected={'DEN-YE-COUNTRY-SCOPE':1,'DEN-YE-FIRST-LEVEL':22,'DEN-YE-GOVERNORATES':21,'DEN-YE-CAPITAL-MUNICIPALITY':1,'DEN-YE-LEGACY-FIRST-LEVEL':21,'DEN-YE-DISTRICTS':333}
 if {r['id']:r['value'] for r in d['denominators']}!=expected: x('YE_DENOMINATORS','denominators','1/22/21/1/21/333 required')
