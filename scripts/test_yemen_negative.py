...
def main():
 ...
 r('YE_DISH_PUBLISHED_FROM_WEAK_SOURCE',lambda d:next(x for x in d['claims'] if x.get('predicate')=='food_dish').update(published=True),'YE_CULTURE_DISHES')
 r('YE_MANDI_CLAIMED_EXCLUSIVE',lambda d:next(x for x in d['claims'] if x.get('predicate')=='food_dish' and x['value']['data'].get('name')=='المندي').update(classification='national'),'YE_SHARED_NOT_EXCLUSIVE')
 r('YE_DIALECT_PROMOTED_TO_VERIFIED',lambda d:next(x for x in d['claims'] if x.get('predicate')=='dialect_profile').update(verification_status='verified'),'YE_CULTURE_DIALECTS')
 r('YE_LANGUAGE_CLAIM_DROPPED',lambda d:d['claims'].__setitem__(slice(None),[x for x in d['claims'] if x.get('predicate')!='language_presence']),'YE_CULTURE_LANGUAGES')
 r('YE_DRESS_PUBLISHED_FROM_WEAK_SOURCE',lambda d:next(x for x in d['claims'] if x.get('predicate')=='clothing_item').update(published=True),'YE_CULTURE_DRESS')
 r('YE_DRESS_CLASSIFICATION_STRIPPED',lambda d:next(x for x in d['claims'] if x.get('predicate')=='clothing_item').update(classification=None),'YE_CULTURE_DRESS')
 r('YE_MARKET_PROMOTED_TO_VERIFIED',lambda d:next(x for x in d['claims'] if x.get('predicate')=='market_presence').update(verification_status='verified'),'YE_CULTURE_CRAFTS_MARKETS')
