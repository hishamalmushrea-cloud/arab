...
def main() -> int:
    ...
    gate.require(len(after_sources) == 17 and before_sources == after_sources, "bahrain_source_idempotence", f"atomic sources={len(after_sources)}/17, unchanged={before_sources == after_sources}")
    ...
    gate.require(negatives.get("status") == "PASS" and negatives.get("detected") == negatives.get("required") == 21, "bahrain_required_mutations", f"detected={negatives.get('detected')}/{negatives.get('required')}")
    gate.require(review.get("status") == "PASS" and review.get("total_sampled") == review.get("total_passed") == 149, "bahrain_full_review_threshold", f"full review passed={review.get('total_passed')}/{review.get('total_sampled')}")
