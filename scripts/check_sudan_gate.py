...
def main():
 g=Gate();before=source_hashes();g.command('sudan_source_refresh',[sys.executable,'scripts/build_sudan_sources.py']);after=source_hashes();g.require(len(after)==4 and before==after,'sudan_source_idempotence',f'atomic sources={len(after)}/4, unchanged={before==after}')
